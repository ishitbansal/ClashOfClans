...
class Cannon(Building):
    def __init__(self, position, V):
        ...
        self.V = V
        self.destroyed = False
        self.health = 60+30*V.building_level
        self.max_health = 60+30*V.building_level
        self.type = 'cannon'
        self.attack = 4+V.building_level
        self.attack_radius = 5+(V.building_level/2)
    ...
    def scan_for_targets(self, King):
        ...
        # Check for stealth archers if they are visible
        for stealth_arch in stealth_archers:
            if (time.time()-stealth_arch.spawn_time >= 10):
                if (stealth_arch.position[0] - self.position[0])**2 + (stealth_arch.position[1] - self.position[1])**2 <= self.attack_radius**2:
                    self.isShooting = True
                    self.attack_target(stealth_arch)
                    return
    ...
    def scan_for_targets(self, King):
        ...
        for stealth_arch in stealth_archers:
            if (time.time()-stealth_arch.spawn_time >= 10):
                if (stealth_arch.position[0] - self.position[0])**2 + (stealth_arch.position[1] - self.position[1])**2 <= self.attack_radius**2:
                    self.isShooting = True
                    self.attack_target(stealth_arch, 0)
                    return
